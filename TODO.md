...
| F-V27 | Academy: trilhas com trava (fricção positiva) + aulas "em breve" | C | S7 | ✅ **Done 10/06/2026** — branch `feat/F-V27-academy-trilhas`, ship dark atrás de `LRP_V2`. Decisão §A.3 = Opção A (ativação por membro). Migration `20260610_f-v27-academy-trilhas.sql` (aditiva, aplicada no remoto). `access_mode`+textos da trava, `member_trail_activations`, `activateTrail`, aulas `available_at`/`is_coming_soon`. SPEC: `docs/sdd/features/F-V27-academy-trilhas/`. E2E manual da trava OK em dev | — |
