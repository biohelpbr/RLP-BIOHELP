...
- [2026-06-05] [RELEASE] **W3 Alterar e-mail do membro pelo admin** (PR #32 `9bb5d9c`, call 05/06, pedido Gabriel). Action `adminUpdateMemberEmail` + seção "E-mail de acesso" no `/admin/community/[id]`: trata UNIQUE(email) com mensagem clara (merge segue manual), atualiza `auth.users` primeiro (rollback se `members` falhar). Substitui o fix via SQL do caso Elaine Violini. E2E: conflito bloqueado, troca persiste, login com novo e-mail OK, antigo barrado.
