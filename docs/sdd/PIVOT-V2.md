...
| F-V23 | Disparo de e-mail nativo no admin (Resend Pro) — segmentação + status | C | P0/P1 | ⏳ **Registrada 02/06/2026 (call)** — mais urgente. Resend Pro contratado. Bloqueio: DKIM/DNS no Cloudflare (anti-spam). SPEC: `docs/sdd/features/F-V23-email-disparo-admin/`. |
| F-V24 | Cancelamento/estorno (Guru webhook + manual; imediato vs renovação) | D | P1 | ⏳ **Registrada 02/06/2026 (call)** — corta acesso RLP+Shopify ao cancelar no Guru. Bloqueio: eventos/payload de cancelamento do Guru (TBD). SPEC: `docs/sdd/features/F-V24-cancelamento-estorno/`. |
| F-V25 | Busca de cliente no admin (`/admin/community`) | B | P1 | ⏳ **Registrada 02/06/2026 (call)** — pedido Gabriel (atendimento). Contract inline TODO §1.1. |
