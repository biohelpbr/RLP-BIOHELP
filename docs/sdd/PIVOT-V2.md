...
| F-V27 | Academy: trilhas com trava (fricção positiva) + aulas "em breve" | C | P2 | ✅ **Done 10/06/2026** — branch `feat/F-V27-academy-trilhas`, ship dark atrás de `LRP_V2`. Decisão §A.3 validada c/ cliente = Opção A (ativação por membro): só BioHelp cria a trava, cada parceira ativa pra si. Migration `20260610_f-v27-academy-trilhas.sql` (aditiva + idempotente, aplicada no remoto). `member_trail_activations` + `activateTrail`; aulas `available_at`/`is_coming_soon`. SPEC: `docs/sdd/features/F-V27-academy-trilhas/`. |
